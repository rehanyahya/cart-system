            ...
            <a-col :span="24">
              <a-form-item label="Country">
                <a-select
                  ...
                  placeholder="Country"
                >
                  <a-select-option v-for='item in countriesData' :key='item.id' :value="item.name"> {{item.name}} </a-select-option>
                </a-select>
              </a-form-item>
            </a-col>
            <a-col :span="12">
...
export default {
  ...
  computed: {
    ...mapState(['cart']),
    ...mapGetters(['getTotalBill', 'getBill']),
  },
  methods: {
    ...
    onSubmit(e) {
      ...
      this.form.validateFields((err, values) => {
        if (!err) {
          console.log(values)
          this.modalMessage = `Hi ${values.first_name} ${values.last_name} your order is placed on address ${values.address} with total bill of $${this.getTotalBill}`;
          this.show();
        }
      });
    },
